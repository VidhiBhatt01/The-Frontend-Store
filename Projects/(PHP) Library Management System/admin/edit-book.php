...
else
{
    ...
    if (isset($_POST['update']))
    {
        ...
        $sql = "update  books set BookName=:bookname,GenreId=:genre,AuthorId=:author,ISBNNumber=:isbn,BookPrice=:price where id=:bookid";
                ...
                <div class="panel panel-info">
                    ...
                    <div class="panel-body">
                        <form role="form" method="post">
                            <?php
                            $bookid = intval($_GET['bookid']);
                            $sql = "SELECT books.BookName, genre.Genre , genre.id as cid,authors.AuthorName, authors.id as athrid,books.ISBNNumber,books.BookPrice,books.id as bookid from books join genre on genre.id=books.GenreId join authors on authors.id=books.AuthorId where books.id=:bookid";
                            $query = $dbh->prepare($sql);
                            $query->bindParam(':bookid', $bookid, PDO::PARAM_STR);
                            $query->execute();
                                    ...
                                    { ?>  
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="genre" required="required">
                                                <option value="<?php echo htmlentities($result->cid); ?>"> <?php echo htmlentities($genname = $result->Genre); ?></option>
                                                <?php
                                                $status = 1;
                                                ...
                                                if ($query1->rowCount() > 0)
                                                {
                                                    foreach ($resultss as $row)
                                                    {
                                                        ...
                                                        {
                                                            ?>  
                                                            <option value="<?php echo htmlentities($row->id); ?>"><?php echo htmlentities($row->Genre); ?></option>
                                                            <?php
                                                        }
                                                    }
                                                } ?> 
